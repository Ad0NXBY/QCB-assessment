...
pPre <- ggplot(long.lcpm) + geom_density(aes(LogPCM, color = SRR_ID)) + #To show before normalization
  labs(title =  "Log CPM Distribution Pre-Filter",
       x = "Log CPM",
       y = "Density")
...
##Create a density plot of the sample distribution -------------------------------------------
pPost <- ggplot(long.lcpm.filtered) + geom_density(aes(LogPCM, color = SRR_ID)) +
  labs(title = "Log CPM Distribution Post-Filter",
       x = "Log CPM",
       y = "Density")


###Arrange the two plots next to each other and display plot
pPrePost <- ggarrange(pPre,pPost,
                      labels = c("A", "B"),
                      ncol = 2, nrow = 1,
                      common.legend = TRUE,
                      legend = "bottom")
...
##PCA Plot-------------------------------------------------------------------------------------
pPCA <- fviz_pca_ind(DS1.svd, repel = FALSE) + 
  expand_limits(x = 6) +
  labs(title = "PCA Plot",
       x = "PC1",
       y = "PC2")
...
ggplot() +
  ...
  geom_point(
    data = filter(DE, FDR < 0.1 & logFC <= -1),
